...
extension RenderingDelegate: WKNavigationDelegate {
    ...
    private func isAllowedNavigation(to url: URL) -> Bool {
        ...
        let permittedSchemes: Set<String> = [
            "http", "https", "about", "blob", "data", "javascript", "file"
        ]
        
        let permittedPrefixes = ["srcdoc", "about:blank", "about:srcdoc"]
        
        return permittedSchemes.contains(scheme) ||
               permittedPrefixes.contains { urlString.hasPrefix($0) } ||
               urlString == "about:blank"
    }
    
    private func launchExternally(_ url: URL) {
    ...
    func webView(
        ...
        manager?.persistenceManager.saveSessions(from: webView)
    }
    ...
    private func applyEnhancements(to interface: WKWebView) {
        let script = """
        (function() {
        ...
        interface.evaluateJavaScript(script) { _, error in
            if let error = error {
                print("Enhancement script error: \(error)")
            }
        }
    }
...
struct SettingsView: View {
    ...
    @State var privacyShow = false
    
    var body: some View {
        NavigationView {
            List {
                Section(header: Text("Display")) {
                    Picker("Consumption Unit", selection: $firebaseService.settings.consumptionUnit) {
                        ForEach(ConsumptionUnit.allCases, id: \.self) { unit in
                            Text(unit.displayName).tag(unit)
                        }
                    }
                    .onChange(of: firebaseService.settings.consumptionUnit) { _ in
                        firebaseService.saveSettings(firebaseService.settings)
                    }
                }
